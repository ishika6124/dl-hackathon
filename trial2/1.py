...
class NormalityEnsemble:
    ...
    def fit(self, X: pd.DataFrame, y: np.ndarray, groups: Optional[np.ndarray] = None) -> None:
        ...
        if proba.shape[1] == 2:
            ...
            # degenerate case
            p_ab = np.zeros(len(y))
        # calibrate threshold on training data with F1 preference
    ...
    def predict_window(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(abnormal_idx) > 0 and self.has_stage2:
            ...
            if len(bearing_global) > 0 and self.has_stage3:
                ...
                out.iloc[bearing_global, out.columns.get_loc("p_inner")] = prob_map.get(1, np.zeros(len(X3)))
                out.iloc[bearing_global, out.columns.get_loc("p_ball")] = prob_map.get(2, np.zeros(len(X3)))
                out.iloc[bearing_global, out.columns.get_loc("p_outer")] = prob_map.get(3, np.zeros(len(X3)))
